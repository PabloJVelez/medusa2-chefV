...
export const PartySizeSelector: FC<PartySizeSelectorProps> = ({ className }) => {
  ...
  const handleInputChange = (value: string) => {
    ...
    if (!isNaN(numValue) && numValue >= MIN_PARTY_SIZE) {
  ...
  return (
    <div className={clsx('space-y-6', className)}>
      ...
      <div className="text-center">
        <h3 className="text-lg font-semibold text-primary-900 mb-2">How Many Guests Will Attend?</h3>
        <p className="text-primary-600">Select the number of guests for your culinary experience.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-primary-900 mb-3">Number of Guests</label>
            ...
            <div className="flex items-center justify-center space-x-4">
              ...
              <button
                ...
                className="w-12 h-12 rounded-full border-2 flex items-center justify-center text-lg font-semibold transition-colors border-accent-500 text-accent-600 hover:bg-accent-50"
              >
                +
              </button>
            </div>

            <p className="text-sm text-primary-600 mt-2 text-center">Minimum {MIN_PARTY_SIZE} guests</p>
          </div>
      ...
      {eventType && price && (
        <div className="bg-accent-50 border border-accent-200 rounded-lg p-4">
          <div className="text-center">
            <h4 className="text-sm font-semibold text-accent-700 mb-2">Pricing Estimate</h4>
            <div className="space-y-1">
              <p className="text-sm text-accent-600">{getEventTypeName()}</p>
              <p className="text-lg font-bold text-accent-800">${price.toFixed(2)} per person</p>
      ...
      {errors.partySize && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-600 text-sm">{errors.partySize.message}</p>
        </div>
      )}
