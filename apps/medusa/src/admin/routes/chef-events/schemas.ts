...
export const chefEventSchema = z.object({
  ...
  requestedDate: z.string().refine((date) => {
    const eventDate = new Date(date);
    const now = new Date();
    now.setHours(0, 0, 0, 0); // Reset to start of day for comparison
    return eventDate >= now;
  }, 'Event date must be today or in the future'),
  requestedTime: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Time must be in HH:MM format'),
...
export const getFormFieldGroups = () => ({
  general: ['requestedDate', 'requestedTime', 'partySize', 'eventType', 'templateProductId', 'estimatedDuration'],
  contact: ['firstName', 'lastName', 'email', 'phone'],
  location: ['locationType', 'locationAddress'],
  details: ['notes', 'specialRequirements', 'totalPrice', 'depositPaid', 'status'],
});
...
export const validationMessages = {
  ...
  partySizeRange: 'Party size must be at least 1',
