...
const toDateTime = (value: unknown) => {
  ...
  if (typeof value === 'string') {
    ...
    // The date stored in DB is timestamptz but we want to treat it as a local date
    const dateOnly = value.split('T')[0];
    return DateTime.fromISO(dateOnly, { zone: 'local' });
  }
  try {
...
export const chefEventToRbc = (e: AdminChefEventDTO): RBCEvent => {
  ...
  const title = [formatTime12Hour(e.requestedTime) || '', fullName || 'Untitled'].filter(Boolean).join(' ');
