...
export const acceptEventWorkflow = createWorkflow(
  ...
  (input: AcceptEventWorkflowInput) => {
    ...
    const templateProduct = transform(
      ...
      async (data, { container }) => {
        ...
        const { data: [templateProduct] } = await query.graph({
          entity: "product",
          fields: ["id", "title", "description", "menu.*", "menu.courses.*", "menu.courses.dishes.*", "variants.id", "variants.price_set.*"],
          filters: {
    ...
    const formattedDate = transform(
      ...
      (data) => DateTime.fromISO(data.event.requestedDate.toString()).toFormat('LLL d, yyyy')
    )
...
• Total Price: ${data.event.totalPrice} ${data.templateProduct.variants?.[0]?.price_set?.prices?.[0]?.currency_code || 'USD'}
      `.trim()
    )
    ...
    const productInput = transform(
      ...
      (data) => ({
        ...
        variants: [{
          ...
          prices: [{
            amount: data.event.totalPrice,
            currency_code: data.templateProduct.variants?.[0]?.price_set?.prices?.[0]?.currency_code || 'USD'
          }]
        }],
        type: "event",
        metadata: {
