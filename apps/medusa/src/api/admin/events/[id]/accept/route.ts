...
export async function GET(
  ...
  try {
    ...
    const formattedDate = DateTime.fromISO(event.requestedDate.toString()).toFormat('LLL d, yyyy');
    ...
    if (!templateProduct?.variants?.[0]?.price_set?.prices?.[0]?.currency_code) {
      throw new Error("Template product is missing currency code")
    }
    const currencyCode = templateProduct.variants[0].price_set.prices[0].currency_code
...
• Total Price: ${event.totalPrice} ${currencyCode}
    `.trim();
    ...
    const chefEvent = {
      ...event,
      requestedDate: event.requestedDate.toString(),
    ...
    await notificationService.createNotifications({
      ...
      data: {
        ...
        booking: {
          ...
          deposit_required: Number(event.totalPrice) * 0.3  
        },
        payment: {
          deposit_amount: Number(event.totalPrice) * 0.3,
