...
export default async function chefEventRequestedHandler({ event: { data }, container }: SubscriberArgs<EventData>) {
  ...
  try {
    ...
    const requestedDate =
      typeof chefEvent.requestedDate === 'string' ? new Date(chefEvent.requestedDate) : chefEvent.requestedDate;
    ...
    // Generate magic link for chef authentication
    const baseUrl = process.env.ADMIN_BACKEND_URL || 'http://localhost:9000';
    ...
    logger.error(
      `Failed to process chef event request for ${data.chefEventId}: ${error instanceof Error ? error.message : String(error)}`,
    );
    throw error;
  }
}
